...
(function() {
  var AUDIO_FILE, GRADIENT, ROWS, container, dancer, data, delta, doTransform, getData, initData, isDrag, k, line, loaded, loadingText, maxTransform, move, noise, origin, pulsar, pulse, settings, spectrum, transform, x, y, _i, _ref;
  ...
  initData = function(ROWS) {
    var data, x, y, _i, _j, _ref, _ref1;
    data = [];
    for (y = _i = 0, _ref = ROWS - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; y = 0 <= _ref ? ++_i : --_i) {
      data.push([]);
      for (x = _j = 0, _ref1 = GRADIENT.length - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; x = 0 <= _ref1 ? ++_j : --_j) {
        data[y].push(noise());
      }
    }
    return data;
  ...
  getData = function() {
    var data, r, step, val, _i, _ref;
    data = [];
    for (r = _i = 0, _ref = GRADIENT.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; r = 0 <= _ref ? ++_i : --_i) {
      if (spectrum !== void 0) {
  ...
  for (k = _i = 0, _ref = data.length - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; k = 0 <= _ref ? ++_i : --_i) {
  ...
  move = function() {
    ...
    data.shift();
    data.push(getData());
    for (q = _j = 0, _ref1 = data.length - 1; 0 <= _ref1 ? _j <= _ref1 : _j >= _ref1; q = 0 <= _ref1 ? ++_j : --_j) {
